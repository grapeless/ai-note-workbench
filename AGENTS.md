...
2. 路由页面统一放在 src/pages/ 下。
    ...
    - 当一级路由页面较复杂时，须在该页面目录内部进行拆分。index.tsx 仅作为路由页面入口，负责组合子组件，不应包含大量业务逻辑或超长 JSX。 页面内部拆出的组件不能放进 pages 下伪装成新的路由页面。
...
8. 避免过度拆分文件和过度设计。仅在代码具有明确的复用价值、独立职责，或当前文件已经明显影响阅读和维护时，才拆分组件、工具函数或模块。禁止将简单 JSX、少量事件处理、单次使用的常量和仅服务于当前组件的简单逻辑拆成独立文件。优先保持相关代码就近维护，不得为了目录“看起来规范”而创建大量只有少量代码的文件。
